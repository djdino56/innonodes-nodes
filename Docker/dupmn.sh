...
function load_profile() {
	...
	local CMD_ARRAY=(COIN_NAME COIN_DAEMON COIN_CLI COIN_FOLDER COIN_CONFIG COIN_GITHUB_WALLET COIN_BOOTSTRAP_FILE COIN_VERSION)
	...
	COIN_GITHUB_WALLET="${prof[COIN_GITHUB_WALLET]}"
	COIN_BOOTSTRAP_FILE="${prof[COIN_BOOTSTRAP_FILE]}"
	COIN_VERSION="${prof[COIN_VERSION]}"
...
function cmd_profadd() {
    ...
    mkdir -p ${prof[COIN_FOLDER]}
    touch ${prof[COIN_FOLDER]}/${prof[COIN_CONFIG]}

    if [[ ! -z "${prof[COIN_GITHUB_WALLET]}" ]]; then
        if [[ ! -f ${prof[COIN_NAME]}-${prof[COIN_VERSION]}.zip ]]; then
            wget ${prof[COIN_GITHUB_WALLET]} -O ${prof[COIN_NAME]}-${prof[COIN_VERSION]}.zip
            echo -e "Retrieved wallet of ${BLUE}$prof_name${NC} and move to ${CYAN}/usr/local/bin/${NC}"
        else
            echo -e "${RED}ERROR:${NC} Found existing wallet file, not downloading the  same ${BLUE}$prof_name${NC} wallet file"
        fi
        unzip -o ${prof[COIN_NAME]}-${prof[COIN_VERSION]}.zip -d /usr/local/bin/
    else
        echo -e "${RED}ERROR:${NC} Can't find COIN_GITHUB_WALLET, not automatically downloading the ${BLUE}$prof_name${NC} wallet"
    fi

    if [[ ! -z "${prof[COIN_BOOTSTRAP_FILE]}" ]]; then
        if [[ ! -f ${prof[COIN_NAME]}-${prof[COIN_VERSION]}-bootstrap.zip ]]; then
            wget ${prof[COIN_BOOTSTRAP_FILE]} -O ${prof[COIN_NAME]}-${prof[COIN_VERSION]}-bootstrap.zip
            echo -e "Retrieved bootstrap of ${BLUE}$prof_name${NC} and move to ${CYAN}${prof[COIN_FOLDER]}${NC}"
        else
